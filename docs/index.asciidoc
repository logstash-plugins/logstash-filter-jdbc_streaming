...
The minimum number of seconds any entry should remain in the cache. You can use decimals
(e.g. `{ "cache_expiration" => 0.25 }`). If there are transient JDBC errors the cache will store empty results for a given
parameter set and bypass the JDBC lookup. This merges the `default_hash` into the event until
the cache entry expires, then the JDB lookup will be tried again for the same parameters.
...
The maximum number of cache entries stored.
